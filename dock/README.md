...
| Security | the constellation: 11 posture dimensions, weakest-link state, coverage and severity counts, every node that has not been attested, and for each attestation whether an independent collector signed it or it rests on the submitting principal's word |
...
| Solar | the digital twin: the estate as a solar system on a Kepler.gl sky with no basemap, with a time axis (scrub any journal record and the plane serves the sky as it stood, referenced at that record's hash), coordination drawn as arcs coloured by status, posture halos on attested bodies, a fidelity panel that counts what the twin has observed, attested, had signed for, bound to the platform and knows nothing about, the plane's own index of what needs attention (unobserved, stale, unavailable, failing or unsigned posture, unbound, awaiting a decision — shown, never recomputed), and drift between the catalog blueprint and the live estate, bindings included. The fabric is drawn as arcs from each bound system to the data platform, coloured by the authority the plane accepted; moons recolour by mode, maturity or approval, with undeclared maturity its own colour rather than a guess; and a body's palette lists every facet the estate records about it — corpus standing, planes, collection policy, methodology, fabric binding, and who vouched for its posture. As a picture: the control plane is the sun, each domain an orbit, each system a body sized by capabilities and coloured by health, each capability a moon coloured by what it may do, relations as arcs. Every verb in its palette — navigate, browse, query, add, edit, retire, observe, secure, control, mine, scan, download — is a governed command the plane already accepts or a read the dock already holds; there is no delete and nothing dispatches |
...
| Console | the six commands the dock may submit — including recording an operator review, whose signals cross the same evidence boundary the server enforces — validated client-side with the control plane's own `validation.js`. The seventh action the plane accepts, binding a system to the fabric, is operator-local and refused over every plane, so no console drafts it |
