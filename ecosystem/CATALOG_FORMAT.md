...
- `surface` and `data_domain` are **closed vocabularies**, declared in [`surfaces.json`](surfaces.json) (19 surfaces) and [`data-domains.json`](data-domains.json) (78 subjects). Each records every spelling it has been written as, and the validator refuses both an unknown value and a recorded alias, naming the canonical one. Left open they produced 28 spellings for 19 surfaces and 84 for 77 subjects; systems that name the same thing differently cannot be queried across.
- `surface`, `method`, `path`, `evidence`, `side_effects`, `cost`, `latency`, `provenance`, `data_domain` and `workflow` on capabilities, and the whole `reference` block, are catalog-only: they never enter the control plane *per capability*. Two of them are drawn from closed vocabularies of bounded values, and the distinct **set** per node does cross, as `metadata.data_domains` and `metadata.surfaces` — enough to ask a snapshot "which systems touch sanctions?" and "which are reachable over MCP?", which is what a coordination layer is for, without the plane accumulating a map of paths and evidence. `cost` is a short hint ("free", "rate-limited 60/min", "GPU minutes"); `latency` a typical figure ("~200 ms", "minutes"); `provenance` where the capability's answers come from ("UN Comtrade capture 2026-08-27", "synthetic:demo", "OpenSanctions bulk CSV"); `data_domain` the data domain it touches (e.g. `trade-flows`, `carrier-identity`, `sanctions`); `workflow` the operator workflow it belongs to (e.g. `daily-chain`, `deploy`).
...
- **`data_domain` and `provenance` are required on every capability of a first-party node**, and refused on an `empty` one. A repository with no code touches no subject and answers nothing; a first-party system that does neither can say so in a sentence. Upstream mirrors are exempt in both directions — annotating someone else's 53 capabilities is a claim about a repository nobody here reads. Coverage: 487 of 634 subjects, 395 of 634 provenances, with the remainder on the mirrors.

- **`maturity` on a capability** is one of `production` · `beta` · `experimental` · `research` · `planned` — the control plane's own vocabulary (`governance/maturity.js`) — declared where someone has assessed the code and absent otherwise, never defaulted: a catalog that wrote `research` onto 634 capabilities nobody had assessed would be a claim with a vocabulary. On an `empty` repository it is derived — `planned`, because there is no code — and a written copy is refused. `methodology_version` names the methodology a capability answers under; a node that declares `reference.methodology` derives it onto every capability that does not name another, and both cross into the journal on the capability.

- **`reference.methodology`** is the corpus owner's versioned statement of how its answers are produced (`notations.corpus-methodology.v1`): `methodology_id`, a semantic `version`, `status` (`research` · `beta` · `production`), `scope`, `source_classes`, `temporal_semantics`, `evidence_model`, `contradiction_handling`, `uncertainty`, the conclusions it refuses to draw (`exclusions`, required and in sentences — a methodology with no negative boundary is a description of a wish), `known_limitations`, and a `changelog` whose last entry is the current version. Only a `hold` or a `transform` may declare one, and never an empty repository or a mirror. It lives here and not in the control plane, which knows no estate; the name and status cross as `metadata.methodology` and `metadata.methodology_status`.

- **`metadata.fabric_layers`** on the platform node names the layers of the data platform it provides — `evidence`, `canonical`, `projection`, `compute` — and a fabric sync binds to it only under an authority that lands in a declared layer. The bindings are `ecosystem/fabric/*.json`, one per system, registered by `ecosystem/fabric.mjs` (operator-local: the plane refuses `register_fabric_sync` over every plane); the plane checks each authority against the system's `reference.corpus.role`.

- **A count of what is in the same file may not also be asserted in it.** `capability_count`, `mcp_tool_count` and `mcp_tools` are refused: they are `capabilities.length` and the non-transport `mcp.*` capabilities, and a written copy can only ever go stale. It had: `control-plane` declared twelve while carrying fourteen. Counts *about another system* — its routes, its layers, its archived files — are observations that cannot be derived here, and those stay.
...
- **`module_family` places every capability in the API architecture** ([API_PLANES.md](../docs/API_PLANES.md)). One of thirteen families; the capability's API role, the planes it may be served on, and the shape its response must take are all derived from it. A first-party node declares it per capability; an upstream mirror may declare `metadata.module_family` once as a default and override per capability. `response_kind` (`referenced` | `operational_observation`) overrides the role's default; `api_exposure: "operator_local"` marks an action that is on no plane at all, and then an operator must approve it and it may not be reached over `mcp`, `agent-tool` or `webhook`; `api_deviation` is a sentence required when a capability's family cannot admit what it does. The rules the validator enforces: never a mutating capability on a public plane, never a governed write that executes without an operator, never a `host_infrastructure` capability claiming a reference it has no root for, and never a family on an empty repository.
